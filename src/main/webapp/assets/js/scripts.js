...
*************************************************** **/
	...
	jQuery(window).ready(function () {
		...
		var userInfo = localStorage.getItem('sf_login_info');
		if (userInfo != null) {
			$('#welcome-user').removeClass('mfp-hide');
			$('#welcome-user-account').removeClass('mfp-hide');

			$('#welcome-login').addClass('mfp-hide');
			$('#welcome-register').addClass('mfp-hide');

			var info = JSON.parse(userInfo);
			$('#welcome-user').html("Welcome, <strong>" + info.firstName + " " + info.lastName + "</strong>");

			$("#email-group").addClass('mfp-hide');
			$("#request-group").removeClass('mfp-hide');

			$("#user-info-group").addClass('mfp-hide');

			$("#contact_name").val(info.firstName + " " + info.lastName);
			$("#contact_email").val(info.email);
		} else {
			$('#welcome-user').addClass('mfp-hide');
			$('#welcome-user-account').addClass('mfp-hide');

			$('#welcome-login').removeClass('mfp-hide');
			$('#welcome-register').removeClass('mfp-hide');

			$("#email-group").removeClass('mfp-hide');
			$("#request-group").addClass('mfp-hide');
			$("#user-info-group").removeClass('mfp-hide');
		}
	});
 ...
 **************************************************************** **/
	...
	if(jQuery('#preloader-bar').length > 0) {

		jQuery(window).load(function() {

			jQuery('#preloader-bar').fadeOut(1000, function() {
				jQuery('#preloader-bar').remove();
			});
	...
	function _animate() {
        ...
        jQuery(".countTo").appear(function(){
			var _t 					= jQuery(this),
				...
				_decimals			= _t.attr('data-decimals')			|| 0;
 ...
 **************************************************************** **/
	function _misc() {
		...
		/** Logout
		 *********************** **/
		jQuery("#logout-btn").bind("click", function(e) {
			localStorage.removeItem("sf_login_info");
		});
 ...
 **************************************************************** **/
	function _form() {
								...
								submitHandler: function(form) {
									...
									jQuery(form).ajaxSubmit({
										...
										error: 		function(data) {
											...
											_toastr("Sent Succeeded!",_Tposition,"success",false);
										},
		...
		$("#register-form").submit(function(e) {
			...
			if (localStorage.getItem('sf_key_' + data.email) == null) {
				localStorage.setItem('sf_key_' + data.email, JSON.stringify(data));
			} else {
				$("#errorMsg").show();
				e.preventDefault();
			}
		...
		$("#login-form").submit(function(e) {
			...
			// saving data into local storage
			var data = localStorage.getItem('sf_key_' + email);
			console.log(data);
			if (data != null) {
				var info = JSON.parse(data);
				console.log(info.password + '_' + password);
				if (info.password == password) {
					localStorage.setItem('sf_login_info', data);
				} else {
					$("#errorMsg").html("<strong>Sorry!</strong> Login Incorrect!");
					$("#errorMsg").show();
					e.preventDefault();
				}
			} else {
				$("#errorMsg").html("<strong>Sorry!</strong> User doesn't exist!");
				$("#errorMsg").show();
				e.preventDefault();
			}
		});
